  ...
  render() {
    ...
    return (
      <View style={[styles.container, { backgroundColor }]}>
        <Image
          source={switchValue ? require('./assets/switch-on.png') : require('./assets/switch-off.png')}

          style={styles.bulbImage}
        />
        <Text style={[styles.textStyle, { color: textColor }]}>AMC IT301: MP1 </Text>
        <Switch
          style={{ marginBottom: 10 }}
          value={switchValue}
          onValueChange={this.handleSwitchToggle}
        />
        <Text style={[styles.textStyle, { color: textColor }]}>{switchValue ? 'SWITCH-ON' : 'SWITCH-OFF'}</Text>
      </View>
    );
  }
